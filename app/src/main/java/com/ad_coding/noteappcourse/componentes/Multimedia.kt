    ...
    val multimediaPickerLauncher = rememberLauncherForActivityResult(
        ...
        if (result.resultCode == Activity.RESULT_OK) {
            ...
            if (clipData != null) {
                ...
                multimediaUris = uris
                onEvent(NoteEvent.MultimediaCambio(multimediaUris)) // Cambiado a MultimediaCambio
            } else {
                result.data?.data?.let { uri ->
                    multimediaUris = listOf(uri.toString())
                    onEvent(NoteEvent.MultimediaCambio(multimediaUris)) // Cambiado a MultimediaCambio
                }
            }
        }
...
//        }
        ...
        if (showDialogForSelection) {
            AlertDialog(
                ...
                title = { Text(text = "Estás seguro de abrir la Multimedia?") },
                confirmButton = {
                    Button(onClick = {
        ...
        if (showDeleteDialog) {
            AlertDialog(
                ...
                title = { Text("Eliminar multimedia") },
                text = { Text("¿Deseas eliminar esta multimedia?") },
                confirmButton = {
                    Button(onClick = {
